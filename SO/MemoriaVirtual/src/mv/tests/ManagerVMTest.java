...
public class ManagerVMTest {
	...
	public void adicionaFrameAcimaLimiteNaMV() {
		...
		assertEquals("verificando limite superior", 2, managerVm.getFrames()
				.size());
	}
	...
	public void adicionaFramesMultiplasThreads3() {
		...
		assertEquals(5, managerVm.getFrames().size());
		assertEquals("adicionaFrameAcimaLimiteDaThread.verificando frames", frames, managerVm.getFrames());
	}

	@Test
	public void testaAreaSwap() {
		managerVm = ManagerVM.getInstance(2, 2);

		Frame frameSwap = new Frame(1, 1);
		
		managerVm.addFrame(frameSwap);
		managerVm.addFrame(new Frame(1, 2));
		managerVm.addFrame(new Frame(1, 3));

		assertEquals("verificando swap", 1, managerVm.getSwap().size());
		assertEquals("verificando frame no swap", true, managerVm.isFrameOnSwap(frameSwap));
	}

	@Test
	public void testaTrocaAreaSwapParaMemoria() {
		managerVm = ManagerVM.getInstance(2, 2);
		
		Frame frameSwap = new Frame(1, 1);
		Frame frameSwap2 = new Frame(1, 2);
		
		managerVm.addFrame(frameSwap);
		managerVm.addFrame(frameSwap2);
		managerVm.addFrame(new Frame(1, 3));
		managerVm.addFrame(new Frame(1, 1));
		...
		assertEquals("verificando swap", 1, managerVm.getSwap().size());
		assertEquals("verificando frame no swap", true, managerVm.isFrameOnSwap(frameSwap2));
		assertEquals(frames, managerVm.getFrames());
	}
}
